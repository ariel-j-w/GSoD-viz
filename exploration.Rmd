...
In general, a widely accepted method for dealing with missing values for the purposes of k-means is to assign the missing value to be the average of the relevant indicator in all other countries. This is fine, but given that our features are all aggregations of smaller variables, I think that we can do a better job of getting more realistic estimates. In general, these aggregation values are left as missing due to one of the sub-variables being missing. Whenever possible, we choosing to reconstruct our relevant missing value, by taking the average of all sub-variables which are not missing.
...
These won't be my final visualizations (those will be in Observable), but I mocked up some real quick ones to see if this was as interesting as I hoped it would be, and it was! Similar viz will definitely make its way into my final product.
